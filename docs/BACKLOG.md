...
- **Statut** : IMPLÉMENTÉ, porté sur la **nouvelle API Deriv**, credentials **vérifiés en lecture seule**, **run live pas encore exécuté**
...
- **Reste à faire (prêt à lancer)** : exécuter 5 A/R sur CRASH1000 puis sur BOOM1000 (`--stake 10 --multiplier 100 --hold 5`), comparer la médiane d'exécution aux 0.745 / 1.063 bps, et reporter le **coût tout compris** dans l'ADR 0021 si le slippage est non négligeable. Profiter de l'accès pour re-tester `active_symbols` / `contracts_for` **authentifié** : le verdict « routes fermées » de l'ADR 0020/0021 repose sur des sondes non authentifiées et sur l'ancienne API — il peut être doublement périmé.
