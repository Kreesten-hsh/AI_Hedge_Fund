...
- **Statut** : IMPLÉMENTÉ, porté sur la **nouvelle API Deriv**, **non exécuté en live** (`DERIV_APP_ID` manquant, voir ci-dessous)
- **Livré** : `scripts/measure_deriv_live_round_trip.py` — authentifie, ouvre une position Multipliers, la ferme après N secondes, écrit `data/measurements/deriv_round_trips.csv`, résume en bps. 77 tests dans `tests/scripts/test_deriv_live_round_trip.py`, **les deux couches doublées** (préambule REST par `httpx.MockTransport`, WebSocket par une connexion doublée), zéro réseau, **99 % de couverture** (la ligne restante est `if __name__ == "__main__"`). `mypy --strict` et `ruff` propres.
...
- **BLOCAGE (réel)** : `DERIV_APP_ID` **absent de `.env`**. La nouvelle API rejette tout PAT sans cet en-tête, et aucun app id essayé n'est accepté (`1089`, `36544`, `16929`, `0`, `1` → `Invalid application`) : `1089` est un identifiant hérité qui n'existe pas sur la nouvelle plateforme. **Action requise côté opérateur** : enregistrer une application sur `home.deriv.com/dashboard` et ajouter `DERIV_APP_ID=<id>` à `.env`. Le CLI sort 2 avec ce message tant que la clé manque. Aucun run live n'a été tenté — et aucun chiffre live n'est reporté ici.
- **Reste à faire une fois `DERIV_APP_ID` renseigné** : exécuter 5 A/R sur CRASH1000 puis sur BOOM1000 (`--stake 10 --multiplier 100 --hold 5`), comparer la médiane d'exécution aux 0.745 / 1.063 bps, et reporter le **coût tout compris** dans l'ADR 0021 si le slippage est non négligeable. Profiter de l'accès pour re-tester `active_symbols` / `contracts_for` **authentifié** : le verdict « routes fermées » de l'ADR 0020/0021 repose sur des sondes non authentifiées et sur l'ancienne API — il peut être doublement périmé.
